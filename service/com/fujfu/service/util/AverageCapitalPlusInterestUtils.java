...
public class AverageCapitalPlusInterestUtils {
	 ...
	 */
	public static double calculatePerMonthPrincipalInterest(double invest, double yearRate, int totalmonth) {
	 ...
	 * @return 每月偿还利息
	 */
	public static Map<Integer, BigDecimal> calculatePerMonthInterest(double invest, double yearRate, int totalmonth) {
	 ...
	 * @return 每月偿还本金
	 */
	public static Map<Integer, BigDecimal> calculatePerMonthPrincipal(double invest, double yearRate, int totalmonth) {
		double monthRate = yearRate / 12;
		BigDecimal monthIncome = new BigDecimal(invest)
				.multiply(new BigDecimal(monthRate * Math.pow(1 + monthRate, totalmonth)))
				.divide(new BigDecimal(Math.pow(1 + monthRate, totalmonth) - 1), 2, BigDecimal.ROUND_HALF_UP);
		Map<Integer, BigDecimal> mapInterest = calculatePerMonthInterest(invest, yearRate, totalmonth);
	...
	public static void main(String[] args) {
		...
		double perMonthPrincipalInterest = calculatePerMonthPrincipalInterest(invest, yearRate, month);
		System.out.println("等额本息---每月还款本息：" + perMonthPrincipalInterest);
		Map<Integer, BigDecimal> mapInterest = calculatePerMonthInterest(invest, yearRate, month);
		System.out.println("等额本息---每月还款利息：" + mapInterest);
		Map<Integer, BigDecimal> mapPrincipal = calculatePerMonthPrincipal(invest, yearRate, month);
		System.out.println("等额本息---每月还款本金：" + mapPrincipal);
		double count = calculateInterestCount(invest, yearRate, month);
		System.out.println("等额本息---总利息：" + count);
		double principalInterestCount = calculatePrincipalInterestCount(invest, yearRate, month);
