...
<body>
	...
	<form id="userWithdrawalListForm"
		action="${pageContext.request.contextPath }/admin/withdrawal/index"
		...
		<table class="table table-bordered table-hover definewidth m10">
			<thead>
				<tr>
					...
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">提现金额</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">提现时间</span></th>
					<th><span class="badge badge-info"
			...
			<c:forEach items="${userWithdrawalList.items }" var="userWithdrawal">
				<tr class="info">
					<td>${userWithdrawal.withdrawalId }</td>
					<td>${userWithdrawal.username }</td>
					<td>${userWithdrawal.amount }</td>
					<td><date:date pattern="yyyy-MM-dd HH:mm"
							value="${userWithdrawal.createTime }"></date:date></td>
					<td><c:choose>
							<c:when test="${userWithdrawal.status == 0 }">处理中</c:when>
							<c:when test="${userWithdrawal.status == 1 }">提现成功</c:when>
							<c:when test="${userWithdrawal.status == 2 }">提现失败</c:when>
							<c:when test="${userWithdrawal.status == 3 }">提现退票</c:when>
							
							<c:otherwise>
							</c:otherwise>
						</c:choose></td>
					<td>${userWithdrawal.billno }</td>
				</tr>
			</c:forEach>
			<tr>
				<td colspan=6><page:Pager formName="userWithdrawalListForm"
						pageAttributeName="userWithdrawalList" styleClass="textfield"
