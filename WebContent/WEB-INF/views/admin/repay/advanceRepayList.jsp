			...
			<table class="table table-bordered table-hover definewidth m10">
				...
				<c:forEach items="${repayList.items }" var="repay">
					<tr class="info">
						...
						<td>${repay.repayDoneCapital }</td>
						<td>${repay.repayDoneInterest }</td>
						<td><c:if test="${repay.status!=1}">
						<a href="${pageContext.request.contextPath }/repay/detail?id=${repay.id }&per=${repay.repayPeriod }">详情</a>
						</c:if>
						<c:if test="${not empty repay.repayDoneTime || repay.status==1}">已还</c:if></td>
					</tr>
				</c:forEach>
				<tr>
				<td colspan=12><page:Pager formName="repayListForm"
