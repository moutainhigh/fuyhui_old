...
<body>
	...
	<div>
		...
		<center><c:if test="${loanApply.isLoans == 0 }"><a class="btn btn-primary" href="${pageContext.request.contextPath }/lender/lending?applyId=${loanApply.id }" onClick="return confirm('是否确定对该项目进行放款?');">放款</a></c:if>
		<c:if test="${loanApply.isLoans == 1 || loanApply.isLoans == 2}">
			<a class="btn btn-primary" href="${pageContext.request.contextPath }/lender/freezing?applyId=${loanApply.id }" onClick="return confirm('是否确定对该项目进行解冻?');">解冻</a>
		</c:if>
		</center><br><br>
	</div>
</body>
