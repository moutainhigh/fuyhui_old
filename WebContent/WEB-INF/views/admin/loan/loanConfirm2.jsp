...
<body>
	...
	<div>
			...
			onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd'})"></td>
				...
				<tr>
					<td width="20%" class="tableleft">投资期限：</td>
				...
				<tr class = "fenqi" style="display:none;">
					...
					<td width="30%"><input readonly id="installmentDate" name="installmentDate" type="text"  value="${loanApply.installmentDate }"></td>
			
					<td width="20%" class="tableleft">线下放款日期：</td>
					<td width="30%"><input disabled id="lDate" name="lDate" type="text"  value="${lDate }" class="Wdate"  
			onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd'})"></td>
			</tr>
			...
			<table class="table table-bordered table-hover definewidth m10">
				...
				<tr>
					<td>融资服务费</td>
					<td><input readonly value="${loanApply.financeServiceFee }" id="financeServiceFee" name="financeServiceFee" type="text"
			...
			<table class="table table-bordered table-hover definewidth m10">
				...
				<tr>
					<td width="20%">发送端：</td>
					<td width="30%" radioid=${loanApply.transmitted} class='radioNmu'><input disabled checked="checked" name="transmitted" type="radio" value=0 />PC+APP&nbsp;
						<input disabled name="transmitted" type="radio" value=1 />PC&nbsp; <input disabled 
				...
				<tr>
					<td width="20%">起投金额：</td>
					<td width="30%"><input value="${loanApply.investMin }" readonly id="investMin" name="investMin" type="text"
						placeholder="请输入起投金额">元</td>
					<td width="20%">递投金额：</td>
					<td width="30%"><input value="${loanApply.proAmount }" readonly id="proAmount" name="proAmount" type="text"
						placeholder="请输入递投金额">元</td>
				</tr>
				<tr><td width="20%">投资开始时间：</td>
				<td width="30%"><input id="sTime" name="sTime" disabled type="text" class="Wdate" value="${startTime }" 
			onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'%y-%M-%d',maxDate:'${dueTime}'})"></td>
					<td width="20%">投资结束时间：</td>
				<td width="30%"><input id="eTime" name="eTime" disabled type="text" class="Wdate" value="${endTime }" 
			onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'sTime\')}',maxDate:'${dueTime}'})"></td>
				...
				<tr>
					...
					<td width="30%" colspan=3><textarea rows="3" cols="20" readonly 
							placeholder="请输入审审核意见">${loanApply.fristAdvice }</textarea></td>
				</tr>
				<tr>
			...
			<form id="loanApplyListForm"  action="">
			<table class="table table-bordered table-hover definewidth m10">
			...
			<tr>
				...
				<tr>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">还款期数</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">应还本金</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">应还利息</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">逾期利息</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">应还总额</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">应还日期</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">实还日期</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">实还本金</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">实还利息</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">状态</span></th>
					<th><span class="badge badge-info"
						style="font-size: 18px; margin-bottom: 10px;">导出兑付结果表</span></th>
					
				</tr>
				<c:forEach items="${applyRepayList.items }" var="allApplyRepayList">
					<tr class="info" ondblclick='window.location.href=""'>
						<td>${allApplyRepayList.repayPeriod }</td>
						<td>${allApplyRepayList.repayCapital }</td>
						<td>${allApplyRepayList.repayInterest }</td>
						<td>0.00</td>
						<td>${allApplyRepayList.repayMoney }</td>
						<td>${allApplyRepayList.repay_req_time }</td>
						<td>${allApplyRepayList.repayDoneTime }</td>
						<td>${allApplyRepayList.repayDoneCapital }</td>
						<td>${allApplyRepayList.repayDoneInterest }</td>
						<td><c:choose>
						<c:when test="${allApplyRepayList.status == 0}">
						待还
						</c:when>
						<c:when test="${allApplyRepayList.status == 1}">
						已还
						</c:when>
						</c:choose></td>
						<td>
						<c:choose>
						<c:when test="${allApplyRepayList.status == 1}">						
							<p style="text-align:right;margin-right:7px;"><a href="/ImportExcel/ImportExcelRecover?id=${allApplyRepayList.applyId }&period=${allApplyRepayList.repayPeriod }" class="down">导出兑付结果表</a></p>
						</c:when>
						<c:otherwise>
							<p style="text-align:right;margin-right:7px;">导出兑付结果表</p>
						</c:otherwise>
						</c:choose>
						</td>
					</tr>
				...
				<tr>
				<td colspan=11><page:Pager formName="applyRepayListForm"
