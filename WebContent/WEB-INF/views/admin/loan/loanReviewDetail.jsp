...
<script type="text/javascript">
		$(function(){
			...
			$('#btn_exit').click(function(){
				var fristAdvice = $("#fristAdvice").val();
				if(fristAdvice == ''){
					alert("请输入初审意见!");
					return false;
				}
		...
		function check(){
			...
			if(fristAdvice == '' || fristAdvice.length >200 ){  
		        alert("输入的初审意见格式不正确！");
		        return false;
		    }
		...
		<form action="${pageContext.request.contextPath }/loan/loanApply/update?isUpdate=0"
			...
			onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd'})"></td>
				...
				<tr class = "fenqi" style="display:none;">
					...
					<td width="30%"><input id="lDate" disabled name="lDate" type="text"  value="${lDate }" class="Wdate"  
			onFocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd'})"></td>
			</tr>
			...
			onfocus="WdatePicker({lang:'zh-cn',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:'#F{$dp.$D(\'sTime\')}',maxDate:'${dueTime}'})"></td>
				...
				<tr>
					...
					<td width="30%" colspan=3><textarea rows="3" cols="20"
							id="fristAdvice" name="fristAdvice" placeholder="请输入审核意见">${loanApply.fristAdvice }</textarea></td>
				</tr>
				<tr>
