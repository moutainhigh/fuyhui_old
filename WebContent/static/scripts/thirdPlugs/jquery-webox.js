...
$.extend({
    webox: function(option) {
        ...
        $(document).ready(function(e) {
            ...
            $('body').append('<div class="background" style="display:none;"></div><div class="webox" style="width:' + width + 'px;height:' + height + 'px;display:none;"><div id="inside" style="height:' + height + 'px;"><h1 id="locked" onselectstart="return false;">' + (option['title'] ? option['title'] : 'webox') + '<a class="span" href="javascript:void(0);" onclick=" $(\'#webox_fade\').remove(); "></a></h1>' + (option['iframe'] ? '<iframe class="w_iframe" src="' + option['iframe'] + '" frameborder="0" width="100%" scrolling="auto" style="border:none;overflow-x:auto;height:' + parseInt(height - 30) + 'px;"></iframe>' : option['html'] ? option['html'] : '') + '</div></div>');
            if (navigator.userAgent.indexOf('MSIE 7') > 0 || navigator.userAgent.indexOf('MSIE 8') > 0) {//$('.webox').css({'filter':'progid:DXImageTransform.Microsoft.gradient(startColorstr=#55000000,endColorstr=#55000000)'});
            }
            if (option['bgvisibel']) {
            ...
            $('#' + option['locked'] + ' .span').click(function() {
                $('.webox iframe').attr('src', 'about:blank');
                $('.webox iframe').remove();
                $('.webox').remove();
                // update 20131023 by zp
                $('.webox').css({
                    display: 'none'
                });
                $('.background').css({
