package com.fujfu.common.payment.fuyou;

import com.fujfu.common.payment.fuyou.pojo.UserWithdrawalBean;
import com.fujfu.common.payment.fuyou.util.FyUtil;
import com.fujfu.common.payment.fuyou.util.SecurityUtils;


/**
 * 用户提现- 管理类
 * @author hjz
 */

public class WithdrawMgt {
	

	/**
	 * 用户提现
	 * @param reqData
	 * @return
	 * @throws Exception
	 */
	public static UserWithdrawalBean withdrawal(UserWithdrawalBean reqData) throws Exception {
		String inputStr=reqData.regSignVal();
		String signatureStr=SecurityUtils.sign(inputStr);
		reqData.setSignature(signatureStr);
		reqData.setAction(FyUtil.WITHDRAWAL);
		System.out.println(signatureStr);
		
		//String backStr=WebUtils.sendHttp(FyUtil.WITHDRAWAL,reqData);
		
		//验签
		//boolean b=SecurityUtils.verifySign(""","");//验签结果
		//System.out.println(b);
		
		//XML转化XMLbean 
		//返回XMLbean
		
		return reqData;
	}
	

	
}
