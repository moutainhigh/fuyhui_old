...
public class CreContrPdf extends PDFDemo{
	/*
	 * 生成投资产品合同
	 * param  
	 */
	public void writePdf2(String path,String productName, LoanApplyVO loanApply, UserVO investuser,UserVO loadApplyUser, LoanInvestmentVO invest, List<ApplyRecoverVO> applyRecoverList) throws Exception {
      ...
      //标题     
        ...
        document.add(head1);
        //加入参数全称产品
        p=convertParToChinese2("投资者在参与投资认购产品","前，应当仔细核对自身是否符合当前产品要求的合格投资者资格条件，并充分了解拟投资认购产品的特点及风险，审慎评估自身的经济状况和财务能力，考虑是否适合参与投资认购，具体包括:",loanApply.getName(),ff10,setting);
        document.add(p);
        document.add(new Chunk("\n"));// 换行
        ...
        document.add(new Chunk("\n"));// 换行
        //加入投资人姓名
        p=convertParToChinese2("本人","对上述《青岛国富金融资产交易中心有限公司合格投资者风险认知书》的内容已经充分理解，承诺本人已阅读产品披露的相关信息，并充分理解和认识产品存在的相关风险，且具备当前拟投资认购产品的合格投资者资格条件。", investuser.getRealname(),ff10,setting);
        document.add(p);
        document.add(new Chunk("\n"));// 换行
        ...
        document.add(new Chunk("\n"));// 换行
        //加入投资人姓名
        p=convertParToChinese2("投资者全称：","",investuser.getRealname(), ff10,250);
        document.add(p);
        //加入投资日期
        p=convertParToChinese2("日      期：  ","",DateUtil.timeMillisToStrNew(loanApply.getLoanTime()), ff10,250);
        document.add(p);
        document.add(new Chunk("\n"));// 换行 
      //金融产品认购协议--------------------------------------------------------------------------------------
        //标题     
        ...
        document.add(head1);
        //加入产品全称
        p=convertParToChinese2("为了明确产品","（以下简称“本产品”）的发行人、认购人的权利义务，保护本产品认购人的合法权益，依据《中华人民共和国民法通则》、《中华人民共和国合同法》、及其他法律法规的有关规定，发行人与认购人本着诚实守信、平等互利、意思表示真实的原则，就本产品认购的有关事宜达成如下协议：",loanApply.getName(), ff10,0);
        document.add(p);
        document.add(new Chunk("\n"));// 换行
        ...
        p=convertParToChinese2("2、“发行人”指","。发行人是在中国境内依法成立的企业法人，并根据其公司章程的规定，经其有权机构相关授权，同意发行人通过青岛国富金融资产交易中心有限公司（以下简称“青金所”）备案、挂牌发行本产品。",loadApplyUser.getCorpName(), ff10,0);
        document.add(p);
        ...
        document.add(p);
        //此处加入产品管理人全称
        p=convertParToChinese2("6、“产品管理人”指受发行人委托，为本产品持有人之利益，在本产品整个产品周期内对本产品进行管理、监督并履行其他法定及约定职责的具备资产管理能力的专业机构。发行人已聘请","为本产品的产品管理人并签订相关受托管理或服务协议。",loanApply.getProductManager(), ff10,0);
        document.add(p);
        ...
        document.add(new Chunk("\n"));// 换行 
        //加入投资人姓名
        p=convertParToChinese2("认购人：  ","",investuser.getRealname(), ff10,250);
        document.add(p);
        //加入投资日期
        p=convertParToChinese2("协议生成日期：","",DateUtil.timeMillisToStrNew(loanApply.getLoanTime()), ff10,250);
        document.add(p);
        document.add(new Chunk("\n"));// 换行 
        ...
        table.addCell(cell);
        cell.setPhrase(new Paragraph(loanApply.getName(), ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph("产品规模", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph(StringUtil.insertComma(loanApply.getAmount().toString(),2)+"元", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph("预期年化收益", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph(StringUtil.handleApr(loanApply.getApr().toString())+"%", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph("回款方式", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph(FeeProperties.REPAYMENT_METHOD.get(loanApply.getPaymentOptions()), ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph("产品期限", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph(loanApply.getActPeriod().toString()+"天", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph("收益起算日", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph(DateUtil.timeMillisToStr(loanApply.getLoanTime(),"yyyy/MM/dd"), ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph("到期日", ff10));
        table.addCell(cell);
        cell.setPhrase(new Paragraph(DateUtil.timeMillisToStr(loanApply.getDueTime(),"yyyy/MM/dd"), ff10));
        table.addCell(cell);
		document.add(table);
	    ...
	    table.addCell(cell);
		    ...
		    cell.setPhrase(new Paragraph(investuser.getUserId().toString(), ff10));
		    table.addCell(cell);
		    cell.setPhrase(new Paragraph(investuser.getRealname(), ff10));
		    table.addCell(cell);
		    cell.setPhrase(new Paragraph(investuser.getCardId(), ff10));
		    table.addCell(cell);
		    cell.setPhrase(new Paragraph(loanApply.getName(), ff10));
		    table.addCell(cell);
		    cell.setPhrase(new Paragraph(invest.getMoney().toString(), ff10));
		    table.addCell(cell);
		document.add(table);
	    ...
	    for(ApplyRecoverVO applyRecover : applyRecoverList){
	    	cell.setPhrase(new Paragraph(DateUtil.timeMillisToStr(applyRecover.getRecoverReqTime(),"yyyy/MM/dd"), ff10));
		    table.addCell(cell);
		    cell.setPhrase(new Paragraph(applyRecover.getRecoverCapital().toString(), ff10));
		    table.addCell(cell);
		    cell.setPhrase(new Paragraph(applyRecover.getRecoverInterest().toString(), ff10));
		    table.addCell(cell);
		    cell.setPhrase(new Paragraph(applyRecover.getRecoverMoney().toString(), ff10));
		    table.addCell(cell);
	    }
	 ...
	 * 生成债权转账转让合同
	 * */
	public void writePdf3(String path,String productName, LoanApplyVO loanApply, UserVO investuser,UserVO loadApplyUser, LoanInvestmentVO invest, List<ApplyRecoverVO> applyRecoverList) throws Exception {
		Document document = new Document();
		 // 生成字体
		PdfWriter.getInstance(document, new FileOutputStream(path));
       ...
       p = convertParToChinese1("协议编号： "+loanApply.getOrderNumber(), f10,0);
       ...
       Chunk chunk = convertChunkByChinesexiahua("深圳市富龙小额贷款有限公司", f10);
       p.add(chunk);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("乙方（受让人）:", f10,0);
       chunk = convertChunkByChinesexiahua(investuser.getRealname(), f10);
       p.add(chunk);     
       p.setAlignment(0);//设置段落居左  
       document.add(p);
       
       p = convertParToChinese1("富元汇用户名:", f10,0);
       chunk = convertChunkByChinesexiahua(String.valueOf(investuser.getUsername()), f10);
       p.add(chunk);
       p.setAlignment(0);//设置段落居左
       document.add(p);      
       
       p = convertParToChinese1("身份证号码:", f10,0);
       chunk = convertChunkByChinesexiahua(investuser.getCardId(), f10);
       p.add(chunk);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       ...
       chunk = convertChunkByChinesexiahua("深圳市富之富信息技术有限公司", f10);
       p.add(chunk);
       p.setAlignment(0);//设置段落居左
       document.add(p);
...
//       String SignedNumber="";
//       String SignedName="";
//      if(StringUtils.isNotEmpty(loanApply.getSignedNumber())){
//         String[] ary = loanApply.getSignedNumber().split(",");//调用API方法按照逗号分隔字符串
//         if(ary.length>1){
//          SignedNumber=ary[0];
//          SignedName=ary[1];
//         }
//       }
       String signedNumber = loanApply.getSignedNumber();
       if(StringUtils.isNotEmpty(signedNumber)) {
    	   		signedNumber="";
       }
    
//       p = convertParToChinese1("1、甲方作为贷款人以其合法资金向借款人提供贷款，签订编号为", ff10,0);
//       chunk = convertChunkByChinesexiahua(signedNumber, ff10);
//       p.add(chunk);
//       chunk=convertChunkByChinese1("的《借款合同》，对借款人享有《借款合同》项下的债权。",ff10);
       chunk=convertChunkByChinese1("1、甲方作为贷款人以其合法资金向借款人提供贷款，签订的《借款合同》，对借款人享有《借款合同》项下的债权。",ff10);
       p.add(chunk);
//       chunk = convertChunkByChinesexiahua("借款合同", ff10);
//       p.add(chunk);
//       chunk=convertChunkByChinese1("",ff10);
//       p.add(chunk);
//       chunk = convertChunkByChinesexiahua("借款合同", ff10);
//       p.add(chunk);
//       chunk=convertChunkByChinese1("》项下的债权。",ff10);
//       p.add(chunk);
       
//       p=convertParToChinese2("《","》，对借款人享有《","》项下的债权",SignedNumber,SignedName,ff10,250);
//       p.setAlignment(0);//设置段落居左
       document.add(p);   
       ...
       chunk = convertChunkByChinesexiahua("https://fuyhui.com/", ff10);
       p.add(chunk);
       ...
       p = convertParToChinese1("1.9  债权回购日：指双方约定由甲方将乙方受让之债权购回之日。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);      
       ...
       table.addCell(cell);
       cell.setPhrase(new Paragraph(StringUtil.insertComma(String.valueOf(invest.getMoney()),2)+"元", ff10));
       table.addCell(cell);
       cell.setPhrase(new Paragraph("债权转让期限", f10));
       table.addCell(cell);
       cell.setPhrase(new Paragraph(DateUtil.timeMillisToStr(loanApply.getLoanTime(),"yyyy年MM月dd日")+" 至 " + DateUtil.timeMillisToStr(loanApply.getDueTime(),"yyyy年MM月dd日"), ff10));
       table.addCell(cell);
       cell.setPhrase(new Paragraph("预期年化收益率", f10));
       table.addCell(cell);
       cell.setPhrase(new Paragraph(StringUtil.handleApr(loanApply.getApr().toString())+"%", ff10));
       table.addCell(cell);
       cell.setPhrase(new Paragraph("收益起算日", f10));
       table.addCell(cell);
       cell.setPhrase(new Paragraph(DateUtil.timeMillisToStr(loanApply.getLoanTime(),"yyyy年MM月dd日"), ff10));
       table.addCell(cell);
	   document.add(table);   
	   ...
	   for(ApplyRecoverVO applyRecover:applyRecoverList){
		   cell.setPhrase(new Paragraph(String.valueOf(applyRecover.getRecoverPeriod()), f10));
		   table.addCell(cell);
		   cell.setPhrase(new Paragraph(DateUtil.timeMillisToStr(applyRecover.getRecoverReqTime(),"yyyy年MM月dd日"), f10));
		   table.addCell(cell);
		   cell.setPhrase(new Paragraph(StringUtil.insertComma(String.valueOf(applyRecover.getRecoverCapital()),2)+"元", f10));
		   table.addCell(cell);
		   cell.setPhrase(new Paragraph(StringUtil.insertComma(String.valueOf(applyRecover.getRecoverInterest()),2)+"元", f10));
		   table.addCell(cell);	   
	   }
	   document.add(table); 
       ...
       p = convertParToChinese1("4.2  甲方按本协议约定对转让债权予以回购，并向乙方支付回购本金及收益后视为乙方将受让债权重新转让回给甲方，甲方即成为债权的权利人，乙方不得再向甲方提出任何请求和主张，并按甲方之要求完成对债务人的相关通知义务。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);   
       ...
       p = convertParToChinese1("5.1.4 甲方应履行本协议第4.1条约定之回购义务，逾期则按逾期金额的每日万分之三之标准向乙方支付逾期费用。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);         
       ...
       document.add(new Chunk("\n"));// 换行
       p = convertParToChinese1("第九条 风险提示", f10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("	受让人在受让债权时应充分了解并认可，任何通过富元汇网站进行的交易并不能完全避免以下风险的产生，富元汇不能也没有义务为如下风险负责：", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("9.1  政策风险。因国家法律、法规、行政规章或政策发生重大调整、变化或其他不可预知的意外事件，可能导致本债权转让的有效性发生变化，从而可能导致受让人无法实现预期收益乃至本金遭受损失。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p); 
       
       p = convertParToChinese1("9.2  行业风险。债权转让标的所处的行业可能会受经济运行周期性变化，以及所处行业的成熟阶段、上下游供求关系、内部竞争程度、替代品威胁等因素的负面影响，从而发生不利于受让人实现预期收益的变化乃至本金遭受损失。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("9.3  市场风险。资金市场供求关系的变化、货币政策、财政政策、行业政策等因素的变化，以及整体经济形势的变化，可能会对利率、汇率、资金成本和商品价格等因素产生负面影响，从而不利于受让人实现预期收益乃至本金遭受损失。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("9.4  信用风险。无论何种原因，当债务方不能按时偿付本金和收益，将导致受让人无法实现预期收益，甚至本金遭受损失。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("9.5  债权转让标的风险。在实际操作中，受让人可能因为受让债权不成功或者提前结束而面临无法实现预期收益的风险以及与再投资有关的风险。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("9.6  操作风险", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（1） 不可预测或无法控制的系统故障、设备故障、通讯故障、停电等突发事故将有可能给各方造成一定损失；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（2） 互联网是全球公共网络，并不受任何一个机构所控制。数据在互联网上传输的途径不是完全确定的，可能会受到非法干扰或侵入。由于存在互联网和移动通讯网络的黑客恶意攻击可能性，各方可能会遭受损失；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（3） 受让人的账号及密码信息可能被盗或被泄露，客户身份可能被仿冒，受让人可能遭受因此导致的损失；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（4） 受让人操作不当等原因可能会造成受让人损失；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（5） 受让人使用的计算机可能因存在性能缺陷、质量问题、计算机病毒、硬件故障及其他原因，而对债权受让造成的交易时间或交易数据方面的影响，给受让人造成损失；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("9.7  其他风险", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（1） 由于自然灾害、战争、法律法规或政策等无法避免或无法控制的因素的出现，将影响市场的正常运行，从而导致债权转让标的损失，甚至影响该债权转让标的的受理、运行、偿付等的正常进行；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（2） 金融市场危机、行业竞争、债务人违约等超出转让人自身直接控制能力之外的风险，也可能导致受让人利益受损；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（3） 债务人因停业、解散、撤销、破产，或者被监管机构撤销相关业务许可等原因不能履行职责时，可能导致受让人的损失；", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("（4） 因其他意外因素和不可抗力（指无法预见、超出合理控制并且尽管加以合理注意仍无法避免的任何情况或事件，应包括但不限于自然之力、火灾、爆炸、地质变化、暴雨、水灾、地震、战争或公敌行动）而导致的风险。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       p = convertParToChinese1("　　以上并不能揭示受让人通过富元汇网站进行受让债权投资的全部风险及市场的全部情形。受让人在做出交易决策前，应通过富元汇网站以及手机应用端公布的信息及其他相关公告了解债权转让标的的风险收益特征，并根据自身的交易目标、风险承受能力和资产状况等谨慎决策，并自行承担全部风险。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
       
       document.add(new Chunk("\n"));// 换行
       p = convertParToChinese1("第十条 其他", f10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);          
       
       p = convertParToChinese1("10.1  各方均确认以其在富元汇网站上的账号、密码登录富元汇网站并进行的任何操作均视为各方的授权行为，各方对通过各自的账号、密码登录富元汇网站进行的任何操作行为承担责任。各方均确认通过富元汇网站对本协议相关条款进行确认以勾选的方式签署，各方认可其效力，并同意无需加盖电子签章。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p); 
       
       p = convertParToChinese1("10.2  富元汇网站债权转让相关规则为本协议的有效补充，与本协议具有同等法律效力,甲乙双方均表示已阅读并同意上述规则。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);     
       
       p = convertParToChinese1("10.3  各方委托丙方保管所有与本协议有关的书面文件或电子信息。若甲、乙方出示的书面文件或电子信息与丙方提供的书面文件或电子信息不一致，均以丙方提供的书面文件或电子信息为准。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);  
       
       p = convertParToChinese1("10.4  本协议与原债权《借款合同》不可分割，任何本协议无约定之事项均应适用《借款合同》之约定。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);                
       
       p = convertParToChinese1("10.5  本协议中部分条款根据相关法律法规的规定成为无效，或部分无效时，该等无效不影响本协议项下其他条款的效力，各方仍应履行其在本协议项下的义务。", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);    
       ...
       p = convertParToChinese1("乙方："+investuser.getRealname(), ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);       
       document.add(new Chunk("\n"));// 换行
       
       
       p = convertParToChinese1("丙方：深圳市富之富信息技术有限公司", ff10,0);
       p.setAlignment(0);//设置段落居左
       ...
       p = convertParToChinese1(DateUtil.timeMillisToStr(loanApply.getLoanTime(),"yyyy年MM月dd日"), ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);           
       
       document.add(new Chunk("\n"));// 换行
       
       p = convertParToChinese1("签订地：深圳市", ff10,0);
       p.setAlignment(0);//设置段落居左
       document.add(p);
