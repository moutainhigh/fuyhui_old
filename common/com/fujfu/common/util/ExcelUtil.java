	...
	private static <T> void fillSheet(WritableSheet sheet, List<T> list, LinkedHashMap<String, String> fieldMap,
		...
		// 填充表头
		 WritableFont font1 = new WritableFont(WritableFont.ARIAL,12,WritableFont.NO_BOLD,false,UnderlineStyle.NO_UNDERLINE,Colour.BLACK);  
	        ...
	        cellFormat1.setAlignment(Alignment.CENTRE); //表头
		for (int i = 0; i < cnFields.length; i++) {
			Label label = new Label(i, 0, cnFields[i],cellFormat1);
			sheet.addCell(label);
		}

		// 填充内容
		int rowNo = 1;
		 WritableFont font2 = new WritableFont(WritableFont.createFont("宋体"),9,WritableFont.NO_BOLD,false,UnderlineStyle.NO_UNDERLINE,Colour.BLACK);  
		  
	        WritableCellFormat cellFormat2 = new WritableCellFormat(font1);
	        cellFormat2.setAlignment(Alignment.CENTRE); //表
		for (int index = firstIndex; index <= lastIndex; index++) {
			...
			for (int i = 0; i < enFields.length; i++) {
				...
				Label label = new Label(i, rowNo, fieldValue,cellFormat2);
				sheet.addCell(label);
			}
